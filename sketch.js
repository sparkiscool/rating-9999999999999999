...
function draw(){
	background("gray")
	timeleft = goal.scale * 100
	playerfunc()
	redfunc()
	bluefunc()
	fill("white")
	text("time: "+timeleft,347,10)
goal.scale = goal.scale - 0.0005
if(goal.scale<0.0005){
		goal.visible = false
}
if(player.isTouching(goal)){
	...
	if(redblock.isTouching(player)||redblock2.isTouching(player)||redblock3.isTouching(player)||redblock4.isTouching(player)||blueblock.isTouching(player)||blackblock.isTouching(player)||blackblock2.isTouching(player)||blackblock3.isTouching(player)||blackblock4.isTouching(player)){
		player.y = 20
		player.x = 20
		blueblock.scale = 1
	}
...
function redfunc(){
	redblock.scale = redblock.scale +0.1
	if(redblock.scale > 5){
		redblock.scale = 1
	}
	redblock2.scale = redblock2.scale +0.1
	if(redblock2.scale > 5){
		redblock2.scale = 1
	}
	redblock3.scale = redblock3.scale +0.1
	if(redblock3.scale > 5){
		redblock3.scale = 1
	}
	redblock4.scale = redblock4.scale +0.1
